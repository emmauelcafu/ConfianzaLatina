...
const HandlerLogin = async (req, res) => {
  ...
  try {
    const { token, role, nombre, id } = await Controllerlogin({ email, password }); // Destructura los datos
    res.json({ mensaje: 'Inicio de sesión exitoso', token, role, nombre, id }); // Incluye el rol, nombre y id en la respuesta
  } catch (error) {
